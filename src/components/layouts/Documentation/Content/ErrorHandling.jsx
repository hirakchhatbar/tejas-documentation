...
import { cleanCode, tejError, enableLogging, convenienceMethods } from '@/data/code-blocks/error-handling/index.js'
    ...
    <div className="flex flex-col gap-10">
      ...
      <div id="zero-config" className="flex flex-col gap-4">
        ...
        <Card className="p-4 border-sky-500/30">
          <p className="font-medium">One of Tejas&apos;s most powerful features is that you don&apos;t need to write any error handling code</p>
          <p className="text-sm text-muted-foreground mt-1">— the framework catches all errors automatically at multiple levels.</p>
        </Card>
        <div id="how-it-works" className="flex flex-col gap-2">
          <h3 className="text-base font-medium">How It Works</h3>
          <p>Tejas wraps all middleware and route handlers with built-in error catching. Any error thrown in your code is automatically: caught by the framework, logged (if exception logging is enabled), and converted to an appropriate HTTP error response.</p>
        </div>
      ...
      <div id="tej-error" className="flex flex-col gap-2">
        ...
        <p>Use <code>TejError</code> for throwing HTTP errors with status codes:</p>
      ...
      <div id="ammo-throw" className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">For more control, use <code>ammo.throw()</code> to send error responses with a status code and optional message or body. The framework will not execute further middleware or the route handler after a response is sent.</p>
      </div>
