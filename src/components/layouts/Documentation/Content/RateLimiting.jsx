...
import {
  ...
  memoryStore,
  redisStore
} from '@/data/code-blocks/rate-limiting/index.js'

const RateLimiting = () => {
  return (
    <div className='flex flex-col gap-10'>
      <p className='text-muted-foreground'>
        Tejas includes a powerful rate limiting system to protect your API from
        abuse. It supports multiple algorithms and storage backends.
      </p>
      ...
      <div id='algorithms' className='flex flex-col gap-4'>
        ...
        <div id='sliding-window' className='flex flex-col gap-2'>
          ...
          <p className='text-sm text-muted-foreground'>
            Best for smooth, accurate rate limiting. Prevents the &quot;burst at
      ...
      <div id='storage-backends' className='flex flex-col gap-4'>
        ...
        <div id='memory' className='flex flex-col gap-2'>
          <h3 className='text-base font-medium'>Memory (Default)</h3>
          <p className='text-sm text-muted-foreground'>
            Good for single-server deployments. Not shared across processes —
            may be inaccurate in distributed setups.
          </p>
          <CodeBlock
            code={memoryStore}
            language='javascript'
        ...
        <div id='redis' className='flex flex-col gap-2'>
          ...
          <p className='text-sm text-muted-foreground'>
            For distributed / multi-instance deployments where counters must be
            shared. The <code>redis</code> npm package is auto-installed on
            first use.
          </p>
          <CodeBlock
            code={redisStore}
      ...
      <div id='best-practices' className='flex flex-col gap-2'>
        ...
        <ul className='ml-6 list-disc space-y-1 text-muted-foreground'>
          <li>
            Use Redis in production for distributed deployments — memory store
            doesn&apos;t share counters across instances
          </li>
          <li>
            Set <code>onRateLimited</code> to return a consistent JSON or
            message
          </li>
          <li>
            Choose sliding-window for accuracy, or fixed-window for simplicity
          </li>
        </ul>
      </div>
    </div>
